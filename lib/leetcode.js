...
export default {
    ...
    getProblem(difficulty) {
        let random_question, url, codeText, commentSymbol;
        ...
        const currentLanguage = editor.getGrammar().name;

        if (editor && currentLanguage === 'Null Grammar') {
                ...
                if (JSON.parse(question.codeArray)) {
                    ...
                    for (let x in question.codeArray) {
                        if (currentLanguage === question.codeArray[x].text) {
                            commentSymbol = commentDict[currentLanguage];
...
`);
                ...
                //                 if (currentLanguage === 'JavaScript' && question.input && question.output && codeText && codeText.match(/var (.*) =/)) {
                //                     var checkQuestion =
                //                         `
                // ${commentSymbol[0]}Save this file and run it to check if you passed the test case!
