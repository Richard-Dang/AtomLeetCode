...
export default {
    ...
    activate() {
        ...
        this.subscriptions.add(atom.commands.add('atom-workspace', {
            ...
            'leetcode:answer': () => this.getAnswer(id),
        }));
    },
    ...
    getProblem(difficulty) {
        ...
        if (this.editor && this.currentLanguage === 'Null Grammar') {
            ...
            self.download(problemList).then((json) => {
                    ...
                    return self.download(discussUrl);

                }).then((discussData) => {

                    const solutionsText = this.getSolutions(discussData);
    ...
    getSolutions(discussData) {
        ...
        async.eachSeries(solutions, (solution, async_cb) => {
            this.download(solution.url).then((solutionData) => {
                ...
                codeSolution.push(this.decodeHtml(solutionData.posts[0].content.replace(/<pre>|<code>|<\/code>|<\/pre>/g, "")));
                //console.log('1:' + codeSolution);
