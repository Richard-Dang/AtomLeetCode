...
`);
        ...
        if (question.input && question.output) {
            if (this.currentLanguage !== 'JavaScript') {
                methodCall = `${this.codeText.match(/\w+\(/)}${question.input})`;
            } else {
                methodCall = `${this.codeText.match(/var (.*) =/)[1].replace(/\w+ =/, '')}(${question.input})`;
            }
...
`);
            ...
            this.editor.insertText(`${this.commentSymbol[0]}Could not find test example`);
        }
    },
